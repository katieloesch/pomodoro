...
const Notification = ({
  ...
  const startNextSession = () => {
    ...
    if (
      activeTimer.type === "session" &&
      sessionLog.session % settings[3].value !== 0
    ) {
      ...
      activeTimer.type === "short-break" ||
      activeTimer.type === "long-break"
    ) {
  ...
  return (
    <div
      className="message"
      style={{ color: activeTimer.colour, backgroundColor: activeTimer.bg }}
    >
      <p> {msg} </p>
      <button
        className="btn-start-next-session"
        style={style}
